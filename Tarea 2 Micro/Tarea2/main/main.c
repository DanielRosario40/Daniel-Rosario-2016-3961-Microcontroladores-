...
static void mqtt5_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        msg_id = esp_mqtt_client_subscribe(client, "/porton/comando", 1);
        ESP_LOGI(TAG, "Suscrito a /porton/comando, msg_id=%d", msg_id);
        break;
...
esp_err_t set_timer(void)
{
    ...
    xTimers = xTimerCreate("Timer", pdMS_TO_TICKS(interval), pdFALSE, (void *)timerId, vTimerCallback);
...
void app_main(void)
{
    ...
    ESP_LOGI(tag, "Iniciando sistema de porton");

    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << fca_pin) | (1ULL << fcc_pin) | (1ULL << ftc_pin) |
                        (1ULL << bc_pin)  | (1ULL << ba_pin)  | (1ULL << bs_pin)  | (1ULL << be_pin),
    ...
    while (1)
    {
        ...
        // 3. LOGICA Y SALIDAS MQTT 
        if (Estado_Actual != Estado_Anterior_Log) {
            const char* msg_estado = "";

            switch(Estado_Actual) {
                case Estado_Inicio:   msg_estado = "Inicio"; break;
                case Estado_Abierto:  msg_estado = "Abierto"; break;
                case Estado_Cerrado:  msg_estado = "Cerrado"; break;
                case Estado_Abriendo: msg_estado = "Abriendo"; break;
                case Estado_Cerrando: msg_estado = "Cerrando"; break;
                case Estado_Stop:     msg_estado = "Stop"; break;
                case Estado_Error:    msg_estado = "Error"; break;
            }

            esp_mqtt_client_publish(mqtt_client, "/porton/estado", msg_estado, 0, 1, 0);
            printf("Estado actual: %s\n", msg_estado);
            
            Estado_Anterior_Log = Estado_Actual;
        }
        ...
        // 5. ESCRITURA DE SALIDAS HARDWARE
